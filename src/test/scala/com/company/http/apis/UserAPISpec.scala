...
class UserAPISpec extends APISpec {
  ...
  trait MainScope extends APISpecScope {
    ...
    implicit val userService = mock[UserService]
    val api = new UserAPI
  ...
  "GET: /users?customerID=ID" should {
    ...
    "return a list of Users for a given customer pk" in new MainScope {
      ...
      Get("/users?customerID=" + customer.id.get) ~> api.routes ~> check {
    ...
    "return an empty list of Users if no Users exists for a given customer pk" in new MainScope {
      ...
      Get("/users?customerID=1") ~> api.routes ~> check {
  ...
  "GET: /users/ID" should {
    ...
    "return a User for a user pk" in new MainScope {
      ...
      Get("/users/" + user.id.get) ~> api.routes ~> check {
